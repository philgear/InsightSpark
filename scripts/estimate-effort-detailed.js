 ...
 * Adapted for InsightSpark / Pivot & Pulse project.
...
const modules = [
    ...
    { name: 'Backend API Proxy (Node)', path: '.', exts: ['server.js'] }
];
...
const effortMultipliers = {
    rely: 1.10, // Required Software Reliability: High (Medical app / PII sanitization)
    data: 1.14, // Data Base Size: High (Local storage structure / state context)
    cplx: 1.17, // Product Complexity: High (D3 Force graph visualization)
    ruse: 1.07, // Developed for Reusability: Nominal
    docu: 1.00, // Documentation Match to Life-Cycle: Nominal
    time: 1.11, // Execution Time Constraint: High (SSE Streaming)
...
function walk(relativeDir, exts) {
    ...
    if (stat.isFile()) {
        ...
        if (exts.some(ext => base === ext || base.endsWith(ext))) {
    ...
    list.forEach(file => {
        ...
        try {
            ...
            if (innerStat && innerStat.isDirectory()) {
                ...
                if (exts.some(ext => file === ext || file.endsWith(ext))) {
...
function getComplexityLabel(slocValue, content) {
    if (slocValue > 400 || content.includes('forceSimulation') || content.includes('GeminiService') || content.includes('D3') || content.includes('express-rate-limit')) {
        return 'High';
    }
...
let markdown = `# COCOMO II Granular File Breakdown (InsightSpark)
