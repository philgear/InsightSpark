...
function getDynamicKsloc() {
    const modules = [
        { name: 'Web Client (Angular SPA)', path: 'src', exts: ['.ts', '.js', '.html', '.css'] },
        { name: 'Backend API Proxy (Express)', path: '.', exts: ['server.js'] }
    ];
    ...
    function walk(relativeDir, exts) {
        ...
        if (stat.isFile()) {
            if (exts.some(ext => resolvedDir.endsWith(ext))) {
    ...
    modules.forEach(mod => {
        ...
        files.forEach(file => {
            try {
                ...
                const ext = path.extname(fullFile).substring(1);
...
const effortMultipliers = {
    rely: 1.10, // Required Software Reliability: High (Medical app / HIPAA PII checking)
    data: 1.14, // Data Base Size: High (Local storage structures / Gemini context)
    ...
    // Platform Factors
    time: 1.11, // Execution Time Constraint: High (Real-time SSE API Streaming)
...
console.log('--- COCOMO II Estimation Results (InsightSpark) ---');
